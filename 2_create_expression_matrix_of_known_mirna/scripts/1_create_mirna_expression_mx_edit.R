...
#' 
#' Identify which rows contain only 0s and which do not:
table(rowSums(dfmeanrc))
rowSums(dfmeanrc)==0
rowsums.zero<-rowSums(dfmeanrc)==0
rowsums.zero
...
#' So,  miRNA profiles contain 0 read counts
#' 
...
#' 
#' "Where does [vector] match in [matrix]?" or "Match the column names of dfmeanrc to the configfile "code" column."
configfile[match(colnames(dfmeanrc),configfile$code),"pigid"]

#' Assign the column names using match:
colnames(dfmeanrc)<- configfile[match(colnames(dfmeanrc),configfile$code),"pigid"]
head(dfmeanrc[1:5])


#' ###6. Transpose the data.frame to make the animals take the rows and the miRNAs the columns
transposedfmeanrc<-t(dfmeanrc)

dim(transposedfmeanrc)

head(transposedfmeanrc[,1:5])

is.numeric(transposedfmeanrc)

#' ## Save data
#' What I am saving here is the unfiltered average read counts in one data.frame
#'

#' ###1. Save the unfiltered average read counts as a data.frame and an Rdata object
write.table(transposedfmeanrc, file = "../1_mean_mature_mirna_expression.txt", quote = FALSE, sep = "\t ", col.names = TRUE)
save(transposedfmeanrc, file = "../1_mean_mature_mirna_expression.Rdata")
