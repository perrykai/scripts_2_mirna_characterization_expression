#' **Script:** `2_filter_target_prediction_results.R`
#' 
...
#' 
#' **Input File(s):** `3_targetscan_miRNA_eQTL_output.txt`
#' 
...
#' 
#' **Output File(s):** `4_filtered_target_prediction_results.Rdata`
#' 
#' **Table of contents:**
#'
...
#' based on target site type, retaining 8mer-1a and 7mer-m8, the two most preferentially conserved and effective classes of target site. 
...
tprst<-read.table("../3_targetscan_miRNA_eQTL_output.txt", 
	sep="\t",
	header=TRUE)
head(tprst)
dim(tprst)
...
save(targets.unique, targets.rst, targets.rst.sum, file="../4_filtered_target_prediction_results.Rdata")